...
> **Note:** This specification is the absolute and definitive record of the Flash protocol. It supersedes and renders obsolete all previous documentation, including "README", "Nod", "Flash v0.1", "Flash v0.2", "Flash v0.3", and "Flash v0.4".
...
To solve the "Dual Consent" hurdle, Flash v0.4.1 introduces **Provisional Mode**.
...
- This turns every first flash into a lead, not a dead end.
...
|----------|-------------|----------|----------------|--------------|-----------------|--------------|
| **Team** | Action | Symmetric | 4 hours | 15 min | 3 | Off |
| **Hierarchical** | Action | Asymmetric | 2 hours | 10 min | 5 | Flash 3+ |
| **Client** | Action | Asymmetric | 2 hours | 10 min | 5 | Flash 3+ |
| **Friend** | Warmth | Symmetric | None | Optional | 3 (soft limit) | Never |
| **Family** | Action (soft) | Asymmetric | 24 hours | 30 min | 5 | Flash 3+ |

- **Symmetric Action** (Team): Either side can initiate a command. Reverse flash is also a command.
- **Asymmetric Action** (Hierarchical, Client, Family): Superior → executor command; executor → superior question.
...
### Global Meaning Library (no typing for first 10 flashes)
...
**Action meanings (command direction):**
- Come here
- I’m outside
- Ready when you are
- Urgent – call me
...
### Custom Meanings (After 10 Flashes)

To address the “rigidity” concern, Flash allows **custom meanings** after a relationship is established:

- **Threshold:** After 10 flashes in a relationship (combined total, both directions), either party can propose a custom meaning.
...
- **Warmth signals:** Remain library‑only to preserve low pressure.

> No custom text for the first 10 flashes ensures friction‑free onboarding. Customisation is earned through repeated use.

---

## 🔄 Asymmetric Reverse Flash (Executor → Superior)

For **Asymmetric Action** templates, the relationship is **not one‑way**. The executor (the person who receives commands) can flash back, but **their flash is always a question or request, never a command**.
...
**For Symmetric Action (Team) templates:** Both sides can send commands. Reverse flash is also a command (no question transformation).
...
| Custom meanings | Not allowed (library only) |
...
**Rules:**
- **Off by default** for personal templates (Friend, Family). User must manually enable in relationship settings.
- **On by default** for professional templates (Team, Hierarchical, Client). Either party can disable it.
...
- **Template type** (e.g., Team → Hierarchical) – requires mutual agreement, because it changes signal class (action vs warmth) and symmetry rules.
...
- Provisional Flashing + Social Graph Auto‑Setup + **Web‑based provisional flashing**.
- Smart Templates (Team, Hierarchical, Client, Friend, Family).
- Global Meaning Library + **Custom meanings after 10 flashes** (mutual consent).
...
- Asymmetric reverse flash (for Hierarchical templates) + Symmetric Action (for Team).
...
- **No typing for first 10 flashes** – library only; customisation earned through repeated use.
- **No setup friction** – provisional mode + templates + social graph auto‑setup + web‑based replies.
...
- **Two action modes** – Symmetric (Team) and Asymmetric (Hierarchical/Client).
...
**Supersedes:** Nod v0.1, Flash v0.1, Flash v0.2, Flash v0.3, Flash v0.4, README
