...
> **Note:** This specification describes the **full Flash protocol** (desktop/mobile apps, native OS integrations).  
> For the initial **web MVP**, certain features (back‑tap gestures, haptic breath, AOD, Dynamic Island) are omitted – see the separate *Web MVP Implementation Guide*.

> **Supersedes:** All previous versions (README, Nod v0.1, Flash v0.1, v0.2, v0.3, v0.4).  
> **Status:** Phase 1 – ready for engineering handoff (core protocol).  
> **Future Phases:** See Roadmap section below.
...
5. **Haptic Breath:** Every 2 minutes while a session is active (waiting for reply), the device emits a faint, gentle haptic pulse. This keeps the session present in peripheral awareness without demanding attention. *(Web MVP: visual breath only.)*
...
**In‑pocket gestures (native apps only; not in web MVP):**
...
## 📱 Zero‑Banner Phone UI (Native Apps)
...
> **Web MVP:** AOD, edge glow, and Dynamic Island are not available; the web app relies on browser‑based notifications and a persistent mini‑orb in the viewport.
...
### Phase 1: Core Protocol (ship now)
- 8‑second Flash Call engine, Binary Soldiers, Melt transitions.
- Provisional Flashing + web‑based fallback + WhatsApp bot.
- Smart Templates (Peer Request, Hierarchical, Client, Friend, Family).
- Global Meaning Library (10 warmth phrases) + custom meanings (Action only, after 7d+5 flashes).
- Ambient Orb (all states), zero‑banner UI, edge glow, back‑tap gestures.
- Asymmetric reverse flash, bilateral immutable history, ping‑back count.
- Group flashes (family/team), contextual badges, post‑acceptance editing.
- Offline queuing, push notifications, expiry from delivery time.

> **Web MVP** implements all of the above except back‑tap, AOD, Dynamic Island, and haptic breath (visual substitutes used).

### Phase 2: OS Integration & Fleeting Warmth
- Dynamic Island, AOD persistent Orb, Apple Watch / Wearables.
- Haptic Signature Library, voice‑to‑flash.
- **Fleeting Warmth Flash** – 1‑second poke + 2‑second ghost window, no soldiers, no orb, vanishing completely.
- **⚡ Reciprocation** – receiver can tap ⚡ during ghost window to send a fleeting flash back, creating a mutual ephemeral moment.
- Daily counter for fleeting flashes (on by default, per relationship).
...
### Future explorations (not yet committed – see `/future_ideas/`)
- **Shy media** – tap‑to‑view 2‑second vanishing photo/video (kept as `shy-media.md`).

---

## 🚫 Discontinued Features (from v0.2 and earlier)

- **Physical Hardware (The Orb):** Discontinued for Phase 1‑2; may be reconsidered later.
...
- **Fleeting warmth (Phase 2)** – 1‑second poke + ghost window + ⚡ reciprocation – a truly zero‑debt presence signal.

> *“Communication shouldn't be a conversation when it only needs to be a Flash.”*

**Version:** v0.5 (final)  
**Status:** Phase 1 ready for engineering handoff  
**Supersedes:** All previous versions (README, Nod v0.1, Flash v0.1, v0.2, v0.3, v0.4)
